...
LOAD_PATH = './user100-agg10'
DUMP_PATH = './'
#IMG_DIR = os.path.join(LOAD_PATH, 'raw/img_align_celeba')
...
def process_data():
    ...
    load_path_train = os.path.join(LOAD_PATH, 'train')
    test_data_dir = os.path.join('data', load_path, 'test')
    load_path_test = os.path.join(LOAD_PATH, 'test')
    ...
    def process_(mode, tf, ratio=0.1, user_lists=None, agg_user=-1):
        #原来是read_path = os.path.join(load_path, mode if mode != 'proxy' else 'train', tf)
        ...
        os.makedirs(os.path.join(dump_path, mode))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agg_user", type=int, default=5, help="number of celebrities to be aggregated together as a device/client (as meta-batch size).")
    parser.add_argument("--ratio", type=float, default=100, help="Number of total celebrities to be sampled for FL training.")
