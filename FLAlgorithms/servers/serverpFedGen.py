...
from utils.model_utils import read_data, read_user_data, aggregate_user_data, create_generative_model,METRICS
import torch
...
class FedGen(Server):
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):       #200次循环迭代
            ...
            if not self.local:
                if glob_iter > 0:
                    for user in self.users:
                        for global_model_param, user_model_param in zip(self.global_model.parameters(),user.model.parameters()):
            ...
            # 由于我们要使用50%的测试数据，所以我们需要重新划分测试集
            test_size = int(0.5 * len(self.x_test))
            x_test_sampled, y_test_sampled = self.x_test[:test_size], self.y_test[:test_size]

            # 评估模型
            loss, accuracy = self.global_model.evaluate(x_test_sampled, y_test_sampled, verbose=2)
        ...
        def update_generator_(n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            ...
            for i in range(n_iters):
                ...
                for user_idx, user in enumerate(self.selected_users):
                    ...
                    user_result_given_gen=user.model.classifier(gen_output)    #出来就是logit

                    user_output_logp_=user.model.soft_predict(user_result_given_gen)
                ...
                ######### get student loss ############
                student_output=student_model.classifier(gen_output)    #出来就是logit
