#!/usr/bin/env python
import argparse
from FLAlgorithms.servers.serveravghkd import FedAvghkd
...
from FLAlgorithms.servers.serverpFed_DataFreeKD import pFed_DataFreeKD
from FLAlgorithms.servers.server_Localtraining import LOCALtra
from FLAlgorithms.servers.serverFedpkd import Fedpkd
...
def create_server_n_user(args, i):
    ...
    if 'Fedhkd' in args.algorithm:
        server = FedAvghkd(args, model, i)
    ...
    elif 'pFed_DataFreeKD' in args.algorithm:
        server = pFed_DataFreeKD(args, model, i)
    elif 'LOCALtra' in args.algorithm:
        server = LOCALtra(args, model, i)
    elif 'Fedpkd' in args.algorithm:
        server = Fedpkd(args, model, i)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="EMnist-alpha0.5-ratio1")#Mnist-alpha0.05-ratio0.5,celeba-user20-agg5   #ration1 数据太多了运行精度太高，GPU内存不足  ratio0.5 数据太少了，运行不出来,别100个用户
    parser.add_argument("--model", type=str, default="cnn")     #mlp,cnn
    parser.add_argument("--train", type=int, default=1, choices=[0,1])
    parser.add_argument("--algorithm", type=str, default="FedAvg")   #LOCALtra,FedAvg,FedAvghkd,Fedhkd,、           Fedpkd，FedProx,FedGen,
    ...
    parser.add_argument("--num_users", type=int, default=80, help="Number of Users per round")
